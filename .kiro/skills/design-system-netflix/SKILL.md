---
name: design-system-netflix
description: >
  Netflix Spain visual style reference — a cinematic, pure-black dark theme with
  a single Netflix-red accent, Netflix Sans typography, generous spacing, and
  content-carousel layouts. Provides ready-to-use design tokens as CSS custom
  properties and Tailwind v4 @theme, component specs (hero CTA, inputs, cards,
  footer), and do/don't rules. Use when building or styling a UI that should look
  like Netflix / a premium cinematic dark streaming interface, or when the user
  asks for these tokens, colors, fonts, or component styles.
license: MIT
---
...
Background: Netflix Red (#e50914). Text: Chalk White (#ffffff). Font: Netflix Sans 700 at 24px. Radius: 16px. Padding: 16px 24px.

### Header Sign-In Button
**Role:** Secondary sign-in action in the header.

Background: Netflix Red (#e50914). Text: Chalk White (#ffffff). Font: Netflix Sans 500 at 14px. Radius: 16px. Padding: 4px 16px.

### Translucent Header Button
**Role:** Language selector and other minor header actions.

Background: rgba(0, 0, 0, 0.4). Text: Chalk White (#ffffff). Border: 1px solid #808080. Radius: 16px. Padding: 6px 16px.

### Hero Email Input
**Role:** The primary email capture field in the hero.

Background: rgba(22, 22, 22, 0.7). Border: 1px solid #5a5a5a. Text: Chalk White (#ffffff) at 16px. Placeholder text: Ash (#808080). Radius: 16px. Padding: 20px 16px.
...
Background: Graphite (#232323). Radius: 16px. Contains an icon, text in Chalk White (#ffffff), and a link-style button.

### Notice Toast
**Role:** Global admin notice shown to signed-in users as a floating toast.

Position: fixed in the bottom-right corner with 24px spacing from the bottom and right edges. Background: glassmorphism — translucent Deep Space (#000000 at ~60% opacity) with a backdrop blur, a 1px border of `color-mix(in oklab, var(--color-chalk-white) 15%, transparent)`, and a soft drop shadow for depth (`0 8px 30px rgba(0,0,0,0.5)`). Text: Chalk White (#ffffff); the Megaphone icon uses the Netflix Red (#e50914) accent. Padding: 12px 24px. Radius: 16px. Max width: 384px (wraps on small screens). Contains a Megaphone icon, the message, and a close (X) button. Enters with a short fade + slide-up; dismissible per session and re-shown when the message changes.
...
- Use rounded corners consistently: 16px for buttons and inputs, 20px+ for cards and media — never sharp or square corners.
...
- Don't use sharp or square corners; every surface (buttons, inputs, cards, chips) must be visibly rounded.
...
3. **Header:** "Design a header for the top of the page on a transparent background. On the left, place the Netflix logo in `#e50914`. On the right, include a 'Sign In' button with a `#e50914` background, `#ffffff` text, 16px radius, and 4px 16px padding."
