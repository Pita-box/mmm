...
export function buildPhotoWatermarkSvg(
  ...
  const estimatedTextWidth = Math.round(layout.fontSizePx * Math.max(1, text.length) * 0.62);
  ...
  return [
    ...
    `<text x="${Math.round(layout.centerX)}" y="${textY}" text-anchor="middle" dominant-baseline="middle" fill="#FFFFFF" fill-opacity="0.94" font-family="Inter, Arial, sans-serif" font-size="${layout.fontSizePx}" font-weight="500">${escapeXml(text)}</text>`,
    "</svg>",
  ].join("");
}
